...
public class FakePlayer extends Module {
   ...
   public final Setting<String> name = this.sgGeneral
      .add(new StringSetting.Builder().name("name").description("The name of the fake player.").defaultValue("seasnail8169").build());
   public final Setting<Boolean> copyInv = this.sgGeneral
      .add(new BoolSetting.Builder().name("copy-inv").description("Copies your inventory to the fake player.").defaultValue(Boolean.valueOf(true)).build());
   public final Setting<Integer> health = this.sgGeneral
      .add(
   ...
   private void fillTable(GuiTheme theme, WTable table) {
      ...
      spawn.action = () -> {
         FakePlayerManager.add(this.name.get(), (float)this.health.get().intValue(), this.copyInv.get());
         table.clear();
         this.fillTable(theme, table);
      };
