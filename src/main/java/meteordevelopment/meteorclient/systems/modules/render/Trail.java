...
public class Trail extends Module {
   ...
   private final Setting<List<ParticleType<?>>> particles = this.sgGeneral
      .add(
         new ParticleTypeListSetting.Builder()
            .name("particles")
            .description("Particles to draw.")
            .defaultValue(ParticleTypes.DRIPPING_OBSIDIAN_TEAR, ParticleTypes.CAMPFIRE_COSY_SMOKE)
            .build()
      );
   private final Setting<Boolean> pause = this.sgGeneral
      .add(
   ...
   private void onTick(TickEvent.Post event) {
      if (!this.pause.get()
         || this.mc.player.getX() != this.mc.player.xo
         || this.mc.player.getY() != this.mc.player.yo
         || this.mc.player.getZ() != this.mc.player.zo) {
