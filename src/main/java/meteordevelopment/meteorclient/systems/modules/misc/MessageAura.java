...
public class MessageAura extends Module {
   ...
   private final Setting<String> message = this.sgGeneral
      .add(new StringSetting.Builder().name("message").description("The specified message sent to the player.").defaultValue("Meteor on Crack!").build());
   ...
   private void onEntityAdded(EntityAddedEvent event) {
      if (event.entity instanceof Player && !event.entity.getUUID().equals(this.mc.player.getUUID())) {
         if (!this.ignoreFriends.get() || this.ignoreFriends.get() && !Friends.get().isFriend((Player)event.entity)) {
