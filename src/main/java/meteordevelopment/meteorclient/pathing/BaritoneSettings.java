...
public class BaritoneSettings implements IPathManager.ISettings {
   ...
   private void createWrappers() {
      ...
      try {
         Class<? extends Settings> klass = (Class<? extends Settings>)BaritoneAPI.getSettings().getClass();
            ...
            if (!Modifier.isStatic(field.getModifiers())) {
               ...
               if (obj instanceof baritone.api.Settings.Setting) {
                  ...
                  if (value instanceof Boolean) {
                     ...
                     Setting<Integer> wrapper = sgInt.add(
                        new IntSetting.Builder()
                           .name(setting.getName())
                           .description(getDescription(setting.getName()))
                           .defaultValue(Integer.valueOf((Integer)setting.defaultValue))
                     ...
                     if (wrapper.name.equals("maxFallHeightNoWater")) {
                        this.noFall = new BoolSetting.Builder()
                           .name(wrapper.name)
                     ...
                     sgInt.add(
                        new IntSetting.Builder()
                           .name(setting.getName())
                           .description(getDescription(setting.getName()))
                           .defaultValue(Integer.valueOf(((Long)setting.defaultValue).intValue()))
   ...
   private static void loadDescriptions() {
      ...
      addDescription(
         "assumeExternalAutoTool", "Disable baritone's auto-tool at runtime, but still assume that another mod will provide auto tool functionality"
      );
      ...
      addDescription(
         "maxPathHistoryLength", "If we are more than 300 movements into the current path, discard the oldest segments, as they are no longer useful"
      );
      addDescription(
