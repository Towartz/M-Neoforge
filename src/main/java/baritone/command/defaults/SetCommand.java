...
public class SetCommand extends Command {
   ...
   public void execute(String label, IArgConsumer args) throws CommandException {
      ...
      if (Arrays.asList("s", "save").contains(arg)) {
      ...
      } else if (Arrays.asList("load", "ld").contains(arg)) {
         ...
         if (paginate) {
            ...
            args.requireMax(1);
            List<? extends Settings.Setting> toPaginate = (viewModified ? SettingsUtil.modifiedSettings(Baritone.settings()) : Baritone.settings().allSettings)
               .stream()
            ...
            Paginator.paginate(
               ...
               settingx -> {
                  ...
                  String commandSuggestion = Baritone.settings().prefix.value + String.format("set %s ", settingx.getName());
            ...
            Settings.Setting<?> setting = Baritone.settings()
               .allSettings
               .stream()
            ...
            if (setting == null) {
               ...
               if (!doingSomething && !args.hasAny()) {
                  this.logDirect(String.format("Value of setting %s:", setting.getName()));
                  this.logDirect(SettingsUtil.settingValueToString(setting));
               } else {
                  ...
                  if (resetting) {
                     ...
                     ((Settings.Setting<Boolean>) (Settings.Setting<?>) setting).value = (Boolean)setting.value ^ true;
                     this.logDirect(String.format("Toggled setting %s to %s", setting.getName(), Boolean.toString((Boolean)setting.value)));
                  } else {
                     ...
                     try {
                        ...
                        throw new CommandInvalidTypeException(args.consumed(), "a valid value", var15);
                     }
                  }
                     ...
                     oldValueComponent.getStyle()
                        ...
                        .withClickEvent(
                           new ClickEvent(
                              ...
                              IBaritoneChatControl.FORCE_COMMAND_PREFIX + String.format("set %s %s", setting.getName(), oldValue)
                           )
                        )
                  );
                  this.logDirect(new Component[]{oldValueComponent});
                  if ((!setting.getName().equals("chatControl") || (Boolean)setting.value || Baritone.settings().chatControlAnyway.value)
                     ...
                     if (setting.getName().equals("prefixControl") && !(Boolean)setting.value) {
                        this.logDirect(
                           "Warning: Prefixed commands will no longer work. If you want to revert this change, use chat control (if enabled) or click the old value listed above.",
                           ChatFormatting.RED
                        );
                     }
                  } else {
                     this.logDirect(
                        "Warning: Chat commands will no longer work. If you want to revert this change, use prefix control (if enabled) or click the old value listed above.",
                        ChatFormatting.RED
                     );
                  }
               }
