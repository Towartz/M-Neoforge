...
public class TunnelCommand extends Command {
   ...
   public void execute(String label, IArgConsumer args) throws CommandException {
      ...
      if (args.hasExactly(3)) {
         ...
         if (width < 1 || height < 2 || depth < 1 || height > this.ctx.world().getMaxBuildHeight()) {
         ...
         if (cont) {
            height--;
            width--;
            Direction enumFacing = this.ctx.player().getDirection();
         ...
         Goal goal = new GoalStrictDirection(this.ctx.playerFeet(), this.ctx.player().getDirection());
