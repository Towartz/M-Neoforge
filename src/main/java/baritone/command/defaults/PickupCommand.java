...
public class PickupCommand extends Command {
   ...
   public void execute(String label, IArgConsumer args) throws CommandException {
      ...
      if (collecting.isEmpty()) {
         this.baritone.getFollowProcess().pickup(stack -> true);
         this.logDirect("Picking up all items");
      } else {
         this.baritone.getFollowProcess().pickup(stack -> collecting.contains(stack.getItem()));
         this.logDirect("Picking up these items:");
         collecting.stream().map(BuiltInRegistries.ITEM::getKey).<String>map(ResourceLocation::toString).forEach(this::logDirect);
      }
   }
