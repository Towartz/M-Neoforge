...
public class BuildCommand extends Command {
   ...
   public void execute(String label, IArgConsumer args) throws CommandException {
      File file0 = args.<File, File, RelativeFile>getDatatypePost(RelativeFile.INSTANCE, this.schematicsDir).getAbsoluteFile();
      File file = file0;
      if (FilenameUtils.getExtension(file0.getAbsolutePath()).isEmpty()) {
         file = new File(file0.getAbsolutePath() + "." + Baritone.settings().schematicFallbackExtension.value);
      }
      ...
      if (!file.exists()) {
         if (file0.exists()) {
            throw new CommandInvalidStateException(
               String.format(
                  "Cannot load %s because I do not know which schematic format that is. Please rename the file to include the correct file extension.", file
               )
            );
         } else {
            throw new CommandInvalidStateException("Cannot find " + file);
         }
      } else if (!SchematicSystem.INSTANCE.getByFile(file).isPresent()) {
