...
public class FindCommand extends Command {
   ...
   public void execute(String label, IArgConsumer args) throws CommandException {
      ...
      Component[] components = toFind.stream()
         .flatMap(
            block -> this.ctx
                  .worldData()
                  .getCachedWorld()
                  .getLocationsOf(BuiltInRegistries.BLOCK.getKey(block).getPath(), Integer.MAX_VALUE, origin.x, origin.y, 4)
                  .stream()
         )
      ...
      if (components.length > 0) {
         Arrays.asList(components).forEach(xva$0 -> this.logDirect(new Component[]{xva$0}));
      } else {
   ...
   public Stream<String> tabComplete(String label, IArgConsumer args) throws CommandException {
      return new TabCompleteHelper()
         .append(CachedChunk.BLOCKS_TO_KEEP_TRACK_OF.stream().<ResourceLocation>map(BuiltInRegistries.BLOCK::getKey).map(Object::toString))
