...
public class FollowCommand extends Command {
   ...
   public void execute(String label, IArgConsumer args) throws CommandException {
      ...
      if (args.hasExactlyOne()) {
         this.baritone.getFollowProcess().follow((group = args.getEnum(FollowCommand.FollowGroup.class)).filter);
      } else {
         args.requireMin(2);
         group = null;
         ...
         this.baritone.getFollowProcess().follow(classes.isEmpty() ? entities::contains : e -> classes.stream().anyMatch(c -> e.getType().equals(c)));
      }
      ...
      if (group != null) {
         ...
         entities.stream().<String>map(Entity::toString).forEach(this::logDirect);
      } else {
