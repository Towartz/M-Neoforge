...
public final class UnpackedSegment {
   ...
   public List<BetterBlockPos> collect() {
      ...
      for (int i = 0; i < path.size(); i++) {
         ...
         if (positionFirstSeen.containsKey(pos)) {
            for (int j = positionFirstSeen.get(pos); i > j; i--) {
               path.remove(i);
            }
         } else {
   ...
   public static UnpackedSegment from(PathSegment segment) {
      return new UnpackedSegment(Arrays.stream(segment.packed).mapToObj(BetterBlockPos::deserializeFromLong), segment.finished);
   }
}
