...
public final class GetToBlockProcess extends BaritoneProcessHelper implements IGetToBlockProcess {
   ...
   public void getToBlock(BlockOptionalMeta block) {
      ...
      this.rescan(new ArrayList<>(), new GetToBlockProcess.GetToBlockCalculationContext(false));
   }
      ...
      if (this.knownLocations.isEmpty()) {
         ...
         if (calcFailed) {
            ...
            if (mineGoalUpdateInterval != 0 && this.tickCount++ % mineGoalUpdateInterval == 0) {
               ...
               CalculationContext context = new GetToBlockProcess.GetToBlockCalculationContext(true);
   ...
   public synchronized boolean blacklistClosest() {
      ...
      this.knownLocations.stream().min(Comparator.comparingDouble(this.ctx.playerFeet()::distSqr)).ifPresent(newBlacklist::add);
      ...
      for (BlockPos pos : this.knownLocations) {
         ...
         if (reachable.isPresent()) {
            ...
            if (this.knownLocations.contains(this.ctx.getSelectedBlock().orElse(null))) {
               ...
               System.out.println(this.ctx.player().containerMenu);
               if (!(this.ctx.player().containerMenu instanceof InventoryMenu)) {
                  return true;
               }
            }
   ...
   private boolean walkIntoInsteadOfAdjacent(Block block) {
      return !Baritone.settings().enterPortal.value ? false : block == Blocks.NETHER_PORTAL;
   }
