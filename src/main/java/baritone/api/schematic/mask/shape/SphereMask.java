...
public final class SphereMask extends AbstractMask implements StaticMask {
   ...
   public SphereMask(int widthX, int heightY, int lengthZ, boolean filled) {
      ...
      this.centerX = (double)widthX / 2.0;
      this.centerY = (double)heightY / 2.0;
      this.centerZ = (double)lengthZ / 2.0;
   ...
   public boolean partOfMask(int x, int y, int z) {
      double dx = Math.abs((double)x + 0.5 - this.centerX);
      double dy = Math.abs((double)y + 0.5 - this.centerY);
      double dz = Math.abs((double)z + 0.5 - this.centerZ);
      return this.outside(dx, dy, dz)
         ? false
         : this.filled || this.outside(dx + 1.0, dy, dz) || this.outside(dx, dy + 1.0, dz) || this.outside(dx, dy, dz + 1.0);
   }

   private boolean outside(double dx, double dy, double dz) {
      return dx * dx / this.radiusSqX + dy * dy / this.radiusSqY + dz * dz / this.radiusSqZ > 1.0;
   }
}
