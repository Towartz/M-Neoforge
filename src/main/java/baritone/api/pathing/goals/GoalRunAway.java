...
public class GoalRunAway implements Goal {
   ...
   public double heuristic(int x, int y, int z) {
      ...
      for (BlockPos p : this.from) {
         double h = GoalXZ.calculate((double)(p.getX() - x), (double)(p.getZ() - z));
         if (h < min) {
            min = h;
         }
      }
   ...
   public double heuristic() {
      int distance = (int)Math.ceil(Math.sqrt((double)this.distanceSq));
   ...
   public boolean equals(Object o) {
      if (this == o) {
         ...
         return this.distanceSq == goal.distanceSq && Arrays.equals((Object[])this.from, (Object[])goal.from) && Objects.equals(this.maintainY, goal.maintainY);
      } else {
         return false;
      }
   }
   ...
   @Override
   public int hashCode() {
      int hash = Arrays.hashCode((Object[])this.from);
      hash = hash * 1196803141 + this.distanceSq;
      return hash * -2053788840 + this.maintainY;
   }
