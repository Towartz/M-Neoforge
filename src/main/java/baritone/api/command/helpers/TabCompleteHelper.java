...
public class TabCompleteHelper {
   ...
   public TabCompleteHelper filterPrefixNamespaced(String prefix) {
      ...
      if (loc == null) {
         ...
         return this.filterPrefix(loc.toString());
      }
   }
   ...
   public TabCompleteHelper addSettings() {
      return this.append(
         BaritoneAPI.getSettings().allSettings.stream().filter(s -> !s.isJavaOnly()).map(Settings.Setting::getName).sorted(String.CASE_INSENSITIVE_ORDER)
      );
   }
   ...
   public TabCompleteHelper addToggleableSettings() {
      return this.append(
         BaritoneAPI.getSettings().getAllValuesByType(Boolean.class).stream().map(Settings.Setting::getName).sorted(String.CASE_INSENSITIVE_ORDER)
      );
   }
}
